...
const FALLBACK_IMAGE = 'https://images.pexels.com/photos/3825527/pexels-photo-3825527.jpeg?auto=compress&cs=tinysrgb&w=800';

export default function LifeScienceSection() {
  const navigate = useNavigate();
  const { articles, isLoading } = useArticlesByCategory('life-science', 6);
  ...
  return (
    <section className="py-16 bg-teal-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold text-dark-teal mb-10">Life Science</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {featured.map((article) => (
            <article key={article.id} className="group">
              <div
                className="rounded-2xl overflow-hidden mb-5 aspect-[16/10] bg-teal-100 cursor-pointer"
              ...
              <div className="flex flex-wrap gap-2">
                {article.tags.slice(0, 2).map((tag, i) => (
                  <button
                    ...
                    className="px-3 py-1 rounded-full text-xs font-medium bg-white text-teal-700 border border-teal-200 hover:bg-teal-50 transition-colors"
                  >
                    {tag}
                  </button>
                ))}
                ...
                <div key={item.id} className="pl-4 border-l-2 border-dark-teal group">
                  ...
                  {item.tags[0] && (
                    <button
                      ...
                      className="inline-block px-3 py-1 rounded-full text-xs font-light bg-white text-teal-600 border border-teal-200 hover:bg-teal-50 transition-colors"
                    >
                      {item.tags[0]}
                    </button>
                  )}
